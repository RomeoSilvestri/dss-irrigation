...
class _RunManager():
    def __init__(self, epochs: int, hparams_dict: dict, name: str, n_statevars: int, n_rules: int, n_input: int, patience: int = 10, delta: int = 0.0001, max_loss_jump: Optional[float] = 0.5):
        ...
        Args:
            ...
            max_loss_jump (float, optional): The maximum allowed percentage jump in loss. Defaults to 0.50 (50%).
        """
        ...
        self.delta = delta
        self.max_loss_jump = max_loss_jump
    ...
    def __call__(self, model_weights, epoch, train_loss, valid_loss, pbar):
        ...
        if math.isnan(loss):
            ...
            loss_jump_percentage = (loss - self.best_loss) / self.best_loss

            if loss_jump_percentage > self.max_loss_jump:
                self.early_stop = True
...
class SANFIS(nn.Module):
    def __init__(self, membfuncs: list, n_input: int, to_device: Optional[str] = None, scale: str = 'None', name: str = 'S-ANFIS'):
        ...
        self._initial_premise = copy.deepcopy(self.premise)
    ...
    def fit(self, train_data: List[torch.Tensor], valid_data: List[torch.Tensor], optimizer: Callable, loss_function: Callable, batch_size: int = 16, shuffle_batches: bool = True, epochs: int = 100, patience: int = 10, delta: float = 1e-5, max_loss_jump: float = 0.5, use_tensorboard: bool = False, logdir: Optional[str] = None, hparams_dict: dict = {}, disable_output: bool = False) -> pd.DataFrame:
        ...
        run_manager = _RunManager(epochs, hparams_dict, self.name, self.n_statevars, self.num_rules,
                                  self.n_input, patience, delta, max_loss_jump)
        # set up tensorboard
        if use_tensorboard:
    ...
    def plotmfs(self, show_initial_weights=True, show_firingstrength: bool = True, bounds: Optional[list] = None, names: Optional[list] = None, title: Optional[str] = None, show_title: bool = True, save_path: Optional[str] = None):
        ...
        mf_colors = plt.rcParams['axes.prop_cycle'].by_key()['color'][:3]
        ...
        with torch.no_grad():
            init_membership_curves = [
                ...
                for i, layer in enumerate(
                    _FuzzyLayer(self._initial_premise).fuzzyfication
                )
            ]
        ...
        # setup plot
        fig, ax = plt.subplots(
            nrows=self.n_statevars, ncols=1, figsize=(6, 10))
        if show_title:
            if title == None:
        ...
        for membfunc in membfuncs:
            if membfunc['function'] == 'gaussian':
                ...
                MembershipLayer = _SigmoidFuzzyLayer(
                    membfunc['params'], membfunc['n_memb'])
            elif membfunc['function'] == 'hybrid':
                MembershipLayer = _HybridFuzzyLayer(membfunc['params'], membfunc['n_memb'])
            else:
                raise Exception(
                    'Membership function must be either "gaussian", "bell", "sigmoid", or "hybrid".')
...
class _HybridFuzzyLayer(nn.Module):
    def __init__(self, params: dict, n_memb: int):
        super(_HybridFuzzyLayer, self).__init__()
        self.params = params
        self.m = n_memb

        self.sigmoid_low = _SigmoidFuzzyLayer(params['low'], 1)
        self.gaussian_center = _GaussianFuzzyLayer(params['center'], n_memb - 2)
        self.sigmoid_high = _SigmoidFuzzyLayer(params['high'], 1)
    ...
    def coeffs(self):
        return {
            ...
            'n_memb': self.m,
            'params': {
                'low': self.sigmoid_low.coeffs,
                'center': self.gaussian_center.coeffs,
                'high': self.sigmoid_high.coeffs
            }
        }

    def reset_parameters(self):
        self.sigmoid_low.reset_parameters()
        self.gaussian_center.reset_parameters()
        self.sigmoid_high.reset_parameters()

    def forward(self, input_):
        low_output = self.sigmoid_low(input_).reshape(input_.shape[0], -1)
        center_output = self.gaussian_center(input_).reshape(input_.shape[0], -1)
        high_output = self.sigmoid_high(input_).reshape(input_.shape[0], -1)

        min_dim = min(low_output.shape[1], center_output.shape[1], high_output.shape[1])
        low_output = low_output[:, :min_dim]
        center_output = center_output[:, :min_dim]
        high_output = high_output[:, :min_dim]

        return torch.cat([low_output, center_output, high_output], dim=1)
...
class _GaussianFuzzyLayer(nn.Module):
    def __init__(self, params: dict, n_memb: int):
        ...
        try:
            ...
            self._mu = torch.tensor([params['params']][0]['mu']['value'])
            self._sigma = torch.tensor([params['params']][0]['sigma']['value'])
...
class _SigmoidFuzzyLayer(nn.Module):
    ...
    def __init__(self, params: dict, n_memb: int):
        ...
        try:
            ...
            self._c = torch.tensor([params['params']][0]['c']['value'])
            self._gamma = torch.tensor([params['params']][0]['gamma']['value'])
